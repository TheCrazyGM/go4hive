...
from .services import get_trending_posts, get_hot_posts, get_post_details, get_account_info, get_account_blog
...
def trending(request):
    ...
    return render(request, 'gopher/trending.html', {'posts': posts, 'title': 'Trending'})
...
def post_detail(request, authorperm):
    ...
    return render(request, 'gopher/post_detail.html', {'post': post, 'replies': replies})
...
def search(request):
    ...
    if query:
        if query.startswith('@'):
            ...
            return render(request, 'gopher/trending.html', {'posts': posts, 'title': f'Tag: {query[1:]}'})
        else:
...
def profile(request, username):
    ...
    if not account:
        return render(request, 'gopher/search.html', {'error': f'ACCOUNT @{username} NOT FOUND.'})
